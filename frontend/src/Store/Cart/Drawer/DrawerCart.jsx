...
export const DrawerCart = (props) => {
  ...
  const increment = (index) => {
    ...
    const newTotalCartPrice = updatedCart.reduce(
  ...
  const updateCartBackend = async () => {
    try {
      const response = await axios.put(`http://127.0.0.1:8000/api/updateCart`, {
        Cart: Cart,
      });
      console.log(response.data);
      setisTrue(true);
    ...
    <div className={`cartSidebar ${!props.isCartOpen ? "hidden" : ""}`}>
        ...
        Cart.map((cart, index) => (
          <div className="itemsdrawer">
            ...
            <div className="imagedrawer">
              {cart.seller_id ? (
                <img
                  src={`http://127.0.0.1:8000/storage/store/collections/${cart.image}`}
                />
              ) : (
            ...
            <IconButton aria-label="hide">
              <i
                id="supprimerdrawer"
                onClick={() => deleteFromCart(cart.cart_id)}
                className="bx bx-x"
              ></i>
            </IconButton>
          </div>
      ...
      {count > 0 ? (
        <Link to="/store/checkout-1">
          <button onClick={updateCartBackend} className="viewcartbtn">
