...
export default function MyWorkouts() {
  ...
  const [workoutsPerPage, setWorkoutsPerPage] = useState(2); // Default to two workouts per page
  ...
  const toggleShowAllExercises = (workoutId) => {
    setShowAllExercises((prev) => {
      ...
      setWorkoutsPerPage(anyOpen ? 1 : 2); // Adjust workoutsPerPage based on any opened workout
      return newShowAll;
    });
  };
  ...
  return (
    <Fragment>
            ...
            currentWorkouts.map((workout) => (
              <div key={workout.id} className={getClassName(workout)}>
                ...
                {workout.workout_exercices.length > 1 && (
                  ...
                  >
                    {showAllExercises[workout.id] ? "Show Less" : "Show More Exercises"}
                  </button>
                )}
              </div>
            </div>
      ...
      <div id="paginationPersonalTraining">
        ...
        <button
          onClick={() => paginate(currentPage + 1)}
          disabled={indexOfLastWorkout >= filteredWorkouts.length}
          id="btnPaginate"
        >
          &gt;
        </button>
